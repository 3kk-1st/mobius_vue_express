...
export default {
  ...
  created() {
    ...
    //connect to get aes
    this.$http.get("/api/mob_ae").then((response) => {
      //on response
    ...
    this.$http.get("/api/mob_cnt").then((response) => {
      //on response
      response.data.forEach(function (obj) {
        console.log(obj.ri);
  ...
  methods: {
    ...
    addNode(id_to_link, name) {
      //id_to_link is id to link to, name is name of node  // change id_to_link to tid?
      let vm = this;
    ...
    removeNode(nodelink) {
      //currently only for removing cin values, removes links automatically
